...
import JobListingDraft from "@/components/sub-components/drafts/JobListingDraft";
import JobListingPa from "@/components/sub-components/pending-approvals/JobListingPa";
import Button from "@/components/ui/button/Button";

export default function Page() {
  const adminLink = process.env.NEXT_PUBLIC_STRAPI_COLLECTION_URL + "api::job-listing.job-listing"
  const [listingData, setListingData] = useState<any[]>();
  useEffect(() => {
    async function loadData() {
      const data: any = await getlListingData(Products.job.searchIndex);
  ...
  return (
    <div>
      <div className="flex justify-between mb-5 ml-5">
        <h2 className="text-lg font-medium text-gray-800 dark:text-white">Job Listing</h2>
