...
export default function BusinessListingDraft() {
  const [pendingApprovalData, setPendingApprovalData] = useState<any[]>();
  useEffect(() => {
    async function loadData() {
      const data: any = await getPendingApprovalListingList(Products.business.searchIndex, true);
      const listingData: any = data[0].hits.map((x: any) => ({
        ...x,
        link: process.env.NEXT_PUBLIC_STRAPI_COLLECTION_URL + "api::business-listing.business-listing/" + x.id
      }));
