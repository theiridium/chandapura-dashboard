...
export default function ClassifiedListingDraft() {
  const [pendingApprovalData, setPendingApprovalData] = useState<any[]>();
  useEffect(() => {
    async function loadData() {
      const data: any = await getPendingApprovalListingList(Products.classifieds.searchIndex, true);
      const listingData: any = data[0].hits.map((x: any) => ({
        ...x,
        link: process.env.NEXT_PUBLIC_STRAPI_COLLECTION_URL + "api::classified-listing.classified-listing/" + x.id
      }));
